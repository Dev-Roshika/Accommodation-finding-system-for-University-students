...
const FileUpdateUpload = () => {
  ...
  const [loadedImages, setLoadedImages] = useState([]); // To store the URLs of already selected images

  const { id } = useParams();
  const navigate = useNavigate();
  const displayedImagesRef = React.createRef();

  useEffect(() => {
    // Fetch the URLs of already selected images when the component mounts
    axios
      .get(`http://localhost:8081/images/update/${id}`)
      .then((res) => {
        setLoadedImages(res.data.selectedImages); // Assuming the server returns an array of image URLs
      })
      .catch((error) => {
  ...
  function deleteLoadedImage(image) {
    // Remove the image from the loadedImages state
    const updatedLoadedImages = loadedImages.filter(
  ...
  const onSubmit = async (e) => {
    ...
    if (selectedImages.length + loadedImages.length > 6) {
    ...
    // });
    // Append loadedImages URLs to the same form data
    loadedImages.forEach((image) => {
      formData.append("loadedImages", image);
    });
    try {
  ...
  return (
    <Fragment>
        ...
        >
          <form onSubmit={onSubmit}>
            ...
            >
              ...
              <h5 className="text-center text-secondary">Up to 6 images</h5>
            </div>
            <div className="p-5">
              <div className="mb-3">
                ...
                <button
                  ...
                  onClick={() => {
                    const displayedImagesElement =
                      document.getElementById("displayedImages");
                    if (displayedImagesElement) {
                      displayedImagesElement.scrollIntoView({
                        behavior: "smooth",
                      });
                    }
                  }}
              ...
              {(selectedImages.length > 0 || loadedImages.length > 0) &&
                (selectedImages.length + loadedImages.length > 6 ? (
                  <div className="mb-3">
                  ...
                  <div className="mb-3">
                    ...
                    >
                      UPLOAD {selectedImages.length + loadedImages.length} IMAGE
                      {selectedImages.length + loadedImages.length === 1
                        ? ""
                        : "S"}
                    </button>
                  </div>
      ...
      <div
        id="displayedImages" // Add this ID
        style={{
          display: "flex",
          flexDirection: "row",
          flexWrap: "wrap",
      ...
      >
        {/* Render the already selected images */}
        {loadedImages &&
          loadedImages.map((image, index) => {
            return (
              ...
              >
                <img src={image} height="200" alt={`Image ${index + 1}`} />
                <button
                  onClick={() => deleteLoadedImage(image)}
                  style={{
                    position: "absolute",
                    bottom: 0,
                    right: 0,
